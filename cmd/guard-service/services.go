...
type services struct {
	...
	//alertReports       map[string]spec.AlertReports // Queue alerts for delivery to Supervisor
...
func (s *services) addAlertReports(record *serviceRecord, podname string, alerts []spec.AlertReport) {
	record.alerts += uint(len(alerts))
	s.supervisor.addAlerts(record.ns, record.sid, podname, alerts)
}
